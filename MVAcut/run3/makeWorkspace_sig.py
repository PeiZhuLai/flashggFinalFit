...
myfile = TFile('/eos/home-p/pelai/HZa/Root_Dataset/run3/Data/run3.root')
...
mvaCuts = {1:0.88, 2:0.94, 3:0.965, 4:0.955, 5:0.97, 6:0.975, 7:0.975, 8:0.975, 9:0.97, 10:0.975, 15:0.97, 20:0.97, 25:0.97, 30:0.96} # use run2 to select run3
# mvaCuts = {5:0.975, 15:0.97, 30:0.96} # train from run3 3 mass points
mvaCut = mvaCuts[mass]

BDT_filename="/afs/cern.ch/work/p/pelai/HZa/HiggsZaAna/HZaMVA/model_Za_BDT_run3.pkl"
...
####################
lumi = {'2022preEE':7.98, '2022postEE':27.01, '2023preBPix':17.61, '2023postBPix':9.53}
# for year in ['2022preEE', '2022postEE','2023preBPix','2023postBPix']:
for year in ['2022preEE']:

    print ("prepare year: "+year)

    filename = '/eos/home-p/pelai/HZa/Root_Dataset/run3/ALP_M{0}/{1}.root'.format(mass, year)
    # filename = '/eos/home-p/pelai/HZa/Root_Dataset/run3/ALP_M{0}/'.format(mass)+'/{0}.root'.format(year)
    ...
    for mass_H in [125 - shift, 125, 125 + shift]:
        ...
        Sqrts = RooRealVar("Sqrts","Sqrts",13.6)
        ...
        for jentry in range(entries):
            ...
            if args.ele:
                if abs(mychain.z_mumu) == 1: 
                    continue
            if args.mu:
                if abs(mychain.z_ee) == 1:
                    continue
