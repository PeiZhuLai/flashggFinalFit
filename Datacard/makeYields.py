...
for year in years:
  ...
  for proc in procs:
    for lep_channel in leps:
      ...
      _inputWS_pattern = f"{inputWSDirMap[year]}/sig/ALP_M{opt.mass_ALP}/ws_Tree2WS/ws_{lep_channel}_{year}.root"
...
if (not opt.skipBkg) & (opt.cat != "NOTAG"):
  ...
  if opt.mergeYears:
    _cat = opt.cat
    _modelWSFile = "%s/%s/CMS-HGG_mva_13p6TeV_multipdf.root"%(opt.bkgModelWSDir,opt.mass_ALP) #Pei-Zhu
    ...
    _inputWSFile = "%s/data/ALP_M%s/ws/run3.root"%(inputWSDirMap[year_for_data],opt.mass_ALP) # Pei-Zhu 
    ...
    for year in years:
      ...
      _proc_s0 = 'ggH' # not needed for data/bkg
      _inputWSFile = "%s/data/ALP_data_bkg_Am%s_workspace.root"%(inputWSDirMap[year],opt.mass_ALP) # Pei-Zhu 
