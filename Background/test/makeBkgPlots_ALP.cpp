...
string catname;
	...
	// plot the data
	TLegend *leg = new TLegend(0.68,0.48,1.1,0.85);
	leg->SetFillColor(0);
	leg->SetLineColor(0);
	leg->SetFillStyle(0);
	leg->SetBorderSize(0);
...
//   hdummy->GetXaxis()->SetTitle("\\mathrm{m}_{\\ell\\ell\\gamma\\gamma} \\ \\mathrm{(GeV)}");//PZ
	...
	hdummy->GetXaxis()->CenterTitle(true);
  	...
  	hdummy->GetYaxis()->SetRangeUser(hdatasub->GetHistogram()->GetMinimum()-1,hdatasub->GetHistogram()->GetMaximum()+1);
  	hdummy->Draw("HIST");
