...
# 改為動態：以中心 y 值 ± 1.5*y 決定範圍；保留為 None 以啟用動態
MEAN_VS_R_YMIN = None
MEAN_VS_R_YMAX = None
...
# 新增：讀入 gaussfit JSON，取出每個模型的 mean 與頂層 exp
def load_gaussfit_json(json_path):
    try:
        ...
        means = {}
        # 優先從 fit_results 下面取 mean
        if isinstance(data.get("fit_results"), dict):
            for k, v in data["fit_results"].items():
...
def main():
    ...
    # 新增：同時帶回 JSON means 與 exp
    json_means, json_exp = {}, None
    if args.mA:
        json_path = os.path.join(BIAS_BASE, "BiasPlots", f"{args.mA}_gaussfit.json")
        json_means, json_exp = load_gaussfit_json(json_path)
        if json_means:
            print(f"[INFO] loaded JSON means from {json_path}")
        if json_exp is not None:
            print(f"[INFO] JSON exp = {json_exp}")
    ...
    points = []
    if json_means:
        # y 軸改為 JSON 的 exp；若缺失則回退到 token 解析的 mu_truth
        ...
        for j, (lab, mean) in enumerate(json_means.items()):
            ...
            points.append((mean, y_val, lab, col))
    else:
    ...
    if points:
        ...
        y_pad = 1.5 * (abs(y_center) if abs(y_center) > 1e-9 else 1.0)
        mg.SetMinimum(y_center - y_pad)
        mg.SetMaximum(y_center + y_pad)
