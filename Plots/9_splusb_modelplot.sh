...
# Output -> Combine/output_splusb_fit_observed/{mA}_splusb_model_{unblind,blind}.pdf
set -e
...
SB=$FFIT/Combine/output_splusb_fit_observed
LUMI="172.13 fb^{-1}"
...
for mA in "${ALPmassList[@]}"; do
    ...
    python3 plot_splusb_model.py "$mA" "$fd" "$SB/${mA}_splusb_model_unblind.pdf" "$LUMI" 0
    python3 plot_splusb_model.py "$mA" "$fd" "$SB/${mA}_splusb_model_blind.pdf"   "$LUMI" 1 115 135
done
echo "[plot] S+B post-fit model plots -> $SB/{mA}_splusb_model_{unblind,blind}.pdf"
