#!/usr/bin/env python3
"""Observed + expected 95% CL limit on mu vs mA (run3), styled like the AN plot.

Reads AsymptoticLimits combine outputs from output_combine_results_observed/ (which
contain observed + all expected quantiles) for mA = 1..30 and draws:
  Observed (solid + markers), Median expected (dashed), 68% (green), 95% (yellow).
...
def read_limits(m):
    f = ROOT.TFile(f"{OBSDIR}/higgsCombine{m}.AsymptoticLimits.mH125.38.root")
    t = f.Get("limit")
    q = {}
...
def main():
    ...
    obs, med = array.array('d'), array.array('d')
    x = array.array('d')
    g68_lo, g68_hi, g95_lo, g95_hi = (array.array('d') for _ in range(4))
    for m in MASSES:
        q = read_limits(m)
        x.append(float(m))
        obs.append(q.get(-1.0, float('nan')))
    ...
    # bands: errors relative to median
    ...
    gobs.SetLineWidth(2); gobs.SetLineColor(ROOT.kBlack)
    gobs.SetMarkerStyle(20); gobs.SetMarkerSize(1.0); gobs.SetMarkerColor(ROOT.kBlack)
    ...
    ymin = min(g95_lo) * 0.6
    ymax = max(max(obs), max(g95_hi)) * 1.6
    frame = c.DrawFrame(0.0, ymin, 32.0, ymax)
    frame.GetXaxis().SetTitle("m_{a} [GeV]")
    frame.GetYaxis().SetTitle("95% CL limit on  #mu")
    ...
    leg = ROOT.TLegend(0.52, 0.66, 0.88, 0.90)
    ...
    out = f"{OUTDIR}/limit_vs_mA_run3_observed"
    c.SaveAs(out + ".pdf"); c.SaveAs(out + ".png")
    print("saved", out + ".pdf/.png")
    print(f"  mA1 observed = {obs[0]:.4f} (expected median {med[0]:.4f})")
