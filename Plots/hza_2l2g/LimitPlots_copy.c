                        ...
                        double& obs,  bool& hasObs)
{
    ...
    if (gSystem->AccessPathName(fname)) {
        cerr << "[readLimitsFromFile] File not found: " << fname << endl;
        return false;
    }
    ...
    f->Close();
    if (q50 < 0) {
        cerr << "[readLimitsFromFile] Median (q50) not found in file: " << fname << endl;
    }
    return (q50 >= 0); // 允许 q50 == 0 的情况
}
...
void BrazilianPlots(int sample=0, bool isInt=true, int year=0, bool APV=false, bool drawObs=false, bool setLimitsOnBR=false)
{
    ...
    auto makeFileName = [](int m) {
        return TString::Format("/afs/cern.ch/work/p/pelai/HZa/flashgg_run3/CMSSW_14_1_0_pre4/src/flashggFinalFit/Combine/output_combine_results/higgsCombine%d.AsymptoticLimits.mH125.38.root", m);
    };
    ...
    int idx = 0;
    std::vector<int> okMasses;
    std::vector<int> failMasses;
    ...
    for (int im=0; im<Nmass; ++im) {
        ...
        if (!readLimitsFromFile(fname, q025, q16, q50, q84, q975, obs, hasObs)) {
            failMasses.push_back(m);
            continue;
        }
        okMasses.push_back(m);

        // 假设乘以某个 xs（若不需要，设为 0.1 pb）
        // 1.0 pb = 1000.0 fb
        // 0.1 pb = 100.0 fb
        double assume_xs = 100.0;
        ...
        // 新增：列印每个 mass 的摘要
        cout << "[BrazilianPlots] m=" << m
             << (setLimitsOnBR? " BR median=" : " XS median=")
             << exp
             << (setLimitsOnBR? "" : " fb")
             << endl;
    ...
    // 新增：若没有任何点，停止
    if (idx == 0) {
        cerr << "[BrazilianPlots] No valid limit points loaded. Check massPoints or input ROOT files path pattern." << endl;
        if (!failMasses.empty()) {
            cerr << "  Tried masses: ";
            for (auto m : failMasses) cerr << m << " ";
            cerr << endl;
        }
        return;
    } else {
        if (!failMasses.empty()) {
            cerr << "[BrazilianPlots] Missing masses: ";
            for (auto m : failMasses) cerr << m << " ";
            cerr << " (these will be skipped)" << endl;
        }
    }
