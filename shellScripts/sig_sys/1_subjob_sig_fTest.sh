...
Executable="$BaseDir/shellScripts/sig_sys/1_runjob_sig_fTest.sh"
LogDir="$BaseDir/Signal/log_files"
SubmitFile="1_subjob_sig_fTest.submit"
  ...
  for iALPmass in "${!ALPmassList[@]}"; do
    for iYear in "${!YearsList[@]}"; do
      for iHmass in "${!HmassList[@]}"; do
        ...
        arguments="${ALPmassList[iALPmass]} ${YearsList[iYear]} ${HmassList[iHmass]} ${ChannelList[iChannel]} $EosDir/mA_M${ALPmassList[iALPmass]}/ws_Tree2WS"
